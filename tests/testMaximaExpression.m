...
e12 = M(1, 2);
assert(isa(e12, 'msym'), 'Expected matrix element to be a sym.');

% end() indexing
e1end = M(1, end);
assert(isa(e1end, 'msym'), 'Expected end-indexed element to be a sym.');
...
dx = diff(expr_diff, x);
assert(isa(dx, 'msym'), 'Expected diff result to be sym.');
dxx = diff(expr_diff, x, 2);  % Second derivative
assert(isa(dxx, 'msym'), 'Expected second derivative to be sym.');
...
assert(isa(result_subs, 'msym'), 'Expected subs result to be sym.');
% Substitute multiple variables
result_subs2 = subs(expr_subs, [x, y], [2, 3]);
assert(isa(result_subs2, 'msym'), 'Expected multi-subs result to be sym.');
